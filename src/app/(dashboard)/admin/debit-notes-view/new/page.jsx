...
const handleUpdateBatch = (batch, requestedQty) => {
  setFormData(prev => {
    const items       = [...prev.items];
    const item        = { ...items[modalItemIndex] };
    const itemQty     = Number(item.quantity);            // total needed
    const batches     = [...item.batches];

    /* How much is already committed on this item? */
    const alreadyAllocated = batches
      .reduce((sum, b) => sum + (b.allocatedQuantity ?? 0), 0);

    /* Block if we would exceed the item quantity */
    if (alreadyAllocated + requestedQty > itemQty) {
      toast.error(`You can’t allocate more than ${itemQty}. 
Currently allocated: ${alreadyAllocated}`);
      return prev;                                        // no change
    }

    /* Locate or create this batch line */
    const idx = batches.findIndex(b => b.batchCode === batch.batchNumber);
    if (idx === -1) {
      /* first allocation from this batch */
      batches.push({
        batchCode:         batch.batchNumber,
        expiryDate:        batch.expiryDate,
        manufacturer:      batch.manufacturer,
        allocatedQuantity: requestedQty,
        availableQuantity: batch.quantity - requestedQty,
      });
    } else {
      /* add to existing allocation */
      const line = { ...batches[idx] };                   // copy
      line.allocatedQuantity += requestedQty;
      line.availableQuantity  = batch.quantity - line.allocatedQuantity;
      batches[idx] = line;
    }

    /* commit changes back into the item & form */
    item.batches            = batches;
    items[modalItemIndex]   = item;
    return { ...prev, items };
  });
};
    ...
    <div className="m-11 p-5 shadow-xl">
      ...
      <div className="mb-6">
        ...
        //   if (!item.managedByBatch) return null;
          if (item.managedByBatch === false) return null;
          return (
