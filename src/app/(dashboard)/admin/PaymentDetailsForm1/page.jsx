...
const PaymentDetailsForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.accountPaidFrom) newErrors.accountPaidFrom = "This field is required.";
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
      ...
      {/* Accounts & Account Paid From Section */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        {[ 
          { label: "Accounts", name: "accounts", type: "text" },
          { label: "Account Paid From", name: "accountPaidFrom", type: "text", required: true },
        ].map(({ label, name, type, required }) => (
          <div key={name}>
            <label className="block text-sm font-medium text-gray-700">{label}</label>
            <input
